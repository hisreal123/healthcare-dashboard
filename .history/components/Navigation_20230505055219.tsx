    ...
    return (
        <>
            <section className="flex space-x-4">
                <div id="sidebar" className=" px-5 h-screen shadow-xl group">
                    ...
                    <nav className="relative pr-5">
                        <ul className="relative">
                            {list.map((list, index) => {
                                return (
                                    <li key={index} className="flex items-center py-3 last:border-t ">
                                        <span className="text-gray-200/70 text-md">{list.icon}</span>
                                        <a href={`./${list.link}`} className="hidden">{list.name}</a>
                                    </li>
                                )
                            })}
                        </ul>
